export async function sendNotification(user, notificationData) {
  ...
  if (!user.notification_url || !user.notification_token) {
    const error = 'User does not have notification settings configured';
    console.error(`❌ ${error}`, { fid: user.fid, hasUrl: !!user.notification_url, hasToken: !!user.notification_token });
