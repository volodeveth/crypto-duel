...
async function handleBattleRoyaleCompleted(battleId, winner, prize) {
  ...
  try {
    ...
    // Send notifications to all players (but limit to avoid spam)
    const maxNotifications = 100; // Limit for BR1000 to avoid overload
    const playersToNotify = players.slice(0, maxNotifications);
    
    for (const playerAddr of playersToNotify) {
      try {
        const user = db.getUserByWallet(playerAddr);
    ...
    if (players.length > maxNotifications) {
      console.log(`ℹ️ Limited notifications to ${maxNotifications} out of ${players.length} players`);
    }
